...
- **多信标连续换向（当前主问题）**：首要根因是 `CR-24` 缺少身份级关联；`CR-29` 会让 Dash 未保持多帧估计方向，`CR-30` 的候选确认也不能保证连续看到的是同一盏灯。
- **dash 偶发走歪、欠冲或过冲**：主要对应 `CR-25` 的开环距离/时间模型和 `CR-29` 的 Dash 方向未锁存。
- **无人机跟踪迟滞、停车后过冲**：主要对应 `DR-16` 缺少速度状态/延迟预测，`DR-15` 进一步增加观测相位误差。
- **快速改变速度时掉高**：优先检查 `DR-18` 的高度估计/控制延迟；`DR-14/DR-17` 会在强横向动作时放大该现象。
...
- 小车 `MCL_car/project/code/imu_car.c:187-196` 遇到全零读取会直接返回，不清除校准完成状态，也不使当前姿态失效。
...
- `MCL_car/project/code/car_image.c:202-276` 只按方向聚类：小于15.5度立即采纳，大角度候选方向一致的state3样本置信度累计到40后切换；它仍无法判断这些方向是否来自同一盏物理信标。

**失败路径**

多盏信标同时亮时，只要“离小车最近”的排名因检测闪烁、投影噪声或小车识别短暂变化而交换，无人机就会立即发布另一盏灯。当前40点置信度能过滤绝大多数短时大角度误选，但同一错误方向持续占优后仍会切换；它证明了方向短时稳定，却不能证明候选身份稳定。
...
- 小车已有可借鉴实现：`MCL_car/project/code/imu_car.c:121-130` 额外按测量重力与预测重力的方向创新降权。
...
无人机快速建立或反转横向速度时，电机差动、姿态建立过程和实际推力动态会产生短时垂向速度。当前倾角补偿只能补偿理想稳态下的推力方向，剩余掉高必须等TOF高度先经过一次低通、差分后再经过第二次低通才会进入速度环，因此至少滞后数个测距帧。现场会表现为快速换向时高度先下降、随后才缓慢追回。

**建议方向**

同步记录原始分区距离、最终高度、爬升率、基础油门、倾角补偿以及目标/实际姿态，区分“测高误判”和“真实垂向扰动”。在确认测高可信后，减少高度与速度估计的重复相位延迟，并借鉴本地 `无名飞控` 的垂向加速度反馈或采用简洁的加速度高频、TOF低频互补估计；不需要直接移植完整EKF。
...
### CR-29：Dash 的多帧 EMA 方向没有锁存到 Dash 结束

**证据**

- `MCL_car/project/code/car_image.c:366-378` 在进入 Dash 前对已采纳方向的单位向量做 EMA，目标是利用多帧数据获得更稳定的 Dash 方向。
- 同文件 `405-418` 只把 EMA 结果放入局部变量 `dash_vx/dash_vy` 并发布一次，没有保存为 Dash 期间专用的持久速度。
- 同文件 `525-526` 在后续视觉包到来且 `dash_end_time > 0` 时，重复发布的是 `visual_last_vx/vy`；这两个变量来自当前采纳方向或正常追点的原始方向，并不保证等于触发时的 EMA 结果。

**失败路径**

触发 Dash 的当次调用先发布 EMA 方向，但下一个无人机数据包到来后，Dash 保持分支会用 `visual_last_vx/vy` 覆盖该目标速度。只要两者存在角度差，多帧 EMA 方向就只生效一个收包周期，Dash 剩余阶段沿另一方向运动，可能表现为极少数 Dash 走歪。Dash 的结束计时仍由 `dash_end_time` 控制，因此该问题主要影响方向，不会延长 Dash 时长。

**建议方向**

触发时将 `dash_vx/dash_vy` 锁存到专用的持久变量，Dash 保持分支只重复发布这组锁存值，并在 Dash 结束、state0 或完整视觉复位时清零。不要继续复用语义会被目标滤波和正常追点改写的 `visual_last_vx/vy`。
...
当前探针高脉冲同时包含主循环工作和期间抢占它的ISR，低脉冲还包含固定400us延时，不能由波形反推出单个ISR耗时或漏拍。高速缓存即使临时启用，也无法区分“TOF测高突然变大导致减油”“真实垂向速度下降”与“姿态/电机响应异常”。因此可能错误排除 `CR-13/DR-18`，或把同一现场现象归到错误模块。
...
- ST 官方资料确认 VL53L8CX 是具有[4x4/8x8独立分区和65°对角FoV](https://www.st.com/en/imaging-and-photonics-solutions/vl53l8cx.html)的深度传感器，[4x4 zone 映射](https://www.st.com/content/st_com/en/technical-documents/DS14161.html)也明确为16个空间区域。当前 `tof.c` 已按对向分区建模，不再把16个距离当作同轴测量。
